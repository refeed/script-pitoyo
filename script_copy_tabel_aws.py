...
from task_id_mapping import METHOD_TASK_ID_TUPLE
    ...
    ** = Unknown
    ...
    dir_source_path = os.path.join(
        ...
        "{year}-{month}-{day}".format(
            year=date.year, month=("%.2d" % date.month), day=("%.2d" % date.day)
        ),
    )
    file_name_to_be_copied = "{year}{month}{day}{hour}**".format(
        year=date.year,
        month=("%.2d" % date.month),
        day=("%.2d" % date.day),
        hour=("%.2d" % time.hour),
        # minute=("%.2d" % (time.minute + 5)),
    )

    full_path_start_name_tobe_copied = os.path.join(
        dir_source_path, file_name_to_be_copied
    )

    filename_found = None
    glob_pattern = (
        os.path.join(dir_source_path, file_name_to_be_copied)
        + "*"
        + str(task_id)
        + ".rih.ascii"
    )
    ...
    if glob_result:
        filename_found = glob_result[0]

    if filename_found is None:
        raise Exception(
            'File "%s" tidak ditemukan' % (glob_pattern)
        )

    path_to_be_copied = filename_found
...
if __name__ == "__main__":
    ...
    for index, row in sumber_table_df.iterrows():
        tanggal = (
            row[0]
            if type(row[0]) is not pandas._libs.tslibs.nattype.NaTType
            else prev_tanggal
        )
        waktu = row[1]

        # HACK
        if None in {tanggal, waktu} or type(waktu) is float:
            continue

        column_num = ord(KOLOM_PRINT_HASIL_START.upper()) - ord("A") + 1

        for task_id in METHOD_TASK_ID_TUPLE:
            try:
            ...
            for arg_location in RAIN_TOTAL_LINE_LOCATION.keys():
                ...
                sumber_table_openpyxl_wb[arg_location].cell(
                    row=index + 2, column=column_num
                ).value = get_rain_total_value(copied_filename, arg_location)
            column_num += 1
